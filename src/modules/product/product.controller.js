...
export const deleteProduct = async (req, res, next) => {
  ...
  if (role === "superAdmin") {
    ...
    await cloudinaryConnection().api.delete_resources_by_prefix(
      `${folderPath}${product.folderId}`
    );
    await cloudinaryConnection().api.delete_folder(
      `${folderPath}${product.folderId}`
    );
    return res.status(200).json({ message: "Delete Done", product });
  }
  const product = await Product.findOneAndDelete({ _id: productId, addedBy });
  if (!product) return next({ message: "Product Not Found", cause: 404 });
  await cloudinaryConnection().api.delete_resources_by_prefix(
    `${folderPath}${product.folderId}`
  );
...
export const getProductsForTwoSpecificBrands = async (req, res, next) => {
  const { brands } = req.body;
  // get products
  const products = await Product.find({brandId: {$in: brands}})
  if (!products.length)
  return next({ message: "No Products For These Brands", cause: 404 });
return res.status(200).json({ message: "Done", count : products.length  , products });
};


// =========================searchOnProductWithAnyField============================//

export const searchOnProductWithAnyField = async (req, res, next) => {
  const { ...search } = req.body;
  // get products
  const features = new APIFeatures(Product.find()).search(search)
  const products = await features.mongooseQuery
  if (!products.length)
