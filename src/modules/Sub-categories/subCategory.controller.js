...
export const updateSubCategory = async (req, res, next) => {
  ...
  if (name) {
    ...
    SubCategoryExist.slug = slugify(name, {replacement : "-" , lower : true , trim : true});
  }
  ...
  if(oldPublicId) {
    ...
    const folderPath = SubCategoryExist.Image.public_id.split(`${SubCategoryExist.folderId}/`)[0]
    ...
    res.status(200).json({ success: true, message: 'subCategory updated successfully', data: SubCategoryExist })
  }
};
...
export const deleteSubCategory = async (req, res, next) => {
    ...
    await cloudinaryConnection().api.delete_resources_by_prefix(`${folderPath}${subCategory.folderId}`)
    await cloudinaryConnection().api.delete_folder(`${folderPath}${subCategory.folderId}`)
